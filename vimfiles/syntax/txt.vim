...
"
" Changelog:
" 1.3 (August 8, 2015)  Add: Dates
"                       Chg: Number formatting
" 1.2 (03-01-2007)
...
" Numbers
" This matches any number, including inside words
" Matching is very fast 
" syn match txtNumber "\v\d(\.\d+)?"

" The alternative is to look for numbers with whitespace around them
" These use positive lookahead, @=
" and positive lookbehind, @<=
" To speed up the matches, the lookahead and lookbehind settings use a "1" to
" mean to only search one byte before or after each matched number
"
syn match txtNumber "\v^[-+]?[0-9.]+(E[-+]\d+)?$"
syn match txtNumber "\v^[-+]?[0-9.]+(E[-+]\d+)?([ ,\t]+)@1="
syn match txtNumber "\v([ ,\t]+)@1<=[-+]?[0-9.]+(E[-+]\d+)?$"
syn match txtNumber "\v([ ,\t]+)@1<=[-+]?[0-9.]+(E[-+]\d+)?([ ,\t]+)@1="

" Third alternative, simply require whitespace before the number
" However, this ends up coloring numbers inside dates of the form 2015-05-08
" which I don't want
" syn match txtNumber "\v[ ,\t]+\d+(\.\d+)?"
...
" Date or date/time
" Dates with slashes
syn match txtDate "\v\d+/\d+/\d+" 
syn match txtDate "\v\d+/\d+/\d+ \d+:\d+(:\d+)?( [AaPp][Mm])?"
...
" Month and day
syn match txtDate "\v(January|February|March|April|May|June|July|August|September|October|November|December) \d+(, \d+)?"
...
" Any of the listed words will be highlighted in yellow
syn keyword txtTodo todo fixme xxx note

" These words will be red 
